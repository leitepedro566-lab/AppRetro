...
- (void)viewDidLoad {
    // 已经彻底删除那个报错的 [super Xcode_is_not_good];
    [super viewDidLoad];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.detailTextLabel.text = [ver[OBF("65787465726E616C5F6964656E746966696572")] stringValue]; // "external_identifier"
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // verifyOwnershipForBundleID:appPath:completion:
    ...
    if ([[ARDowngradeManager sharedManager] respondsToSelector:verifySel]) {
        void (^verifyBlock)(BOOL, NSString *, NSString *, NSArray *) = ^(BOOL isMatch, NSString *purchaser, NSString *active, NSArray *allAccounts) {
            ...
            if (isMatch) {
                ...
                NSString *msg = [NSString stringWithFormat:OBF("E8B4A6E58FB7E4B88DE58CB9E9858D200A25400A2540"), purchaser, active]; // "账号不匹配 \n%@\n%@"
                UIAlertController *mismatchAlert = [UIAlertController alertControllerWithTitle:OBF("E9AA8CE8AF81E5A4B1E8B4A5") message:msg preferredStyle:UIAlertControllerStyleAlert]; // "验证失败"
                
                [mismatchAlert addAction:[UIAlertAction actionWithTitle:OBF("E58F96E6B688") style:UIAlertActionStyleCancel handler:nil]]; // "取消"
                [mismatchAlert addAction:[UIAlertAction actionWithTitle:OBF("E58887E68DA2E8B4A6E58FB7") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) { // "切换账号"
                    
                    // 调出设备多账户切换单选栏
                    UIAlertController *switchSheet = [UIAlertController alertControllerWithTitle:OBF("E98089E68B9CE8B4A6E58FB7") message:purchaser preferredStyle:UIAlertControllerStyleActionSheet]; // "选择账号"
                    
                    for (NSString *accName in allAccounts) {
                        NSString *btnTitle = accName;
                        if ([accName caseInsensitiveCompare:purchaser] == NSOrderedSame) {
                            btnTitle = [NSString stringWithFormat:OBF("2A20E58887E68DA2E588B03A202540"), accName]; // "* 切换到: %@"
                        }
                        [switchSheet addAction:[UIAlertAction actionWithTitle:btnTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction *act) {
                            
                            // 动态调用隐藏的 executeAccountSwitchToName:
...
- (void)executeDowngradeProcessWithVersionStr:(NSString *)verStr versionID:(long long)vId {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:OBF("E7A1AEE8AEA4E9998DE7BAA7") message:msg preferredStyle:UIAlertControllerStyleAlert]; // "确认降级"
    ...
    [alert addAction:[UIAlertAction actionWithTitle:OBF("E5BC80E5A78BE9998DE7BAA7") style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) { // "开始降级"
        ...
        // 完美防闪退设计：延时 0.5 秒，等 UIAlertController 销毁动画完全结束后再挂起程序退出桌面。
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
            id app = [NSClassFromString(OBF("55494170706C69636174696F6E")) performSelector:NSSelectorFromString(OBF("7368617265644170706C69636174696F6E"))]; // UIApplication sharedApplication
